<template>
  <div class="assets-center-container">
    <div style="background: #fafafa;border:2px solid #f2f5fa;height: 50px;">
      <el-form :inline="true" :model="formInline" style="margin-left: 15px; margin-top: 7px">
        <el-form-item label="">
    ...
    <div class="meta" />
    <div class="assets-center-container el-table__body-wrapper">
      <template v-if="tableColumns.length">
        ...
        >
          <el-table-column type="index" width="80" label="序号" fixed />
          <el-table-column
          ...
          <el-table-column label="操作" fixed="right" width="280">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="text" icon="el-icon-view" @click="monitorTableData(scope.row)">监控</el-button>
              <el-button size="mini" type="text" icon="el-icon-delete" @click="deleteData(scope.row.id, scope.$index, tableList)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
...
import { PhysicalServer as masterApi } from '@/api/data-map'
...
import DrawerMonitor from '../../monitor/components/drawer_monitor'
  ...
  methods: {
    ...
    getList() {
      masterApi.list({
        ...
        this.classifyId = res.data.result.classify_id
    ...
    addNewData() {
      if (this.classifyId === 0) {
        this.$message.error('classify id 未找到')
        return false
      }
