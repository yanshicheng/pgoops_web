<template>
  <div class="data-center-tablemanage">
      ...
      <div v-for="(items, indexs) in mainData" :key="indexs">
        <div class="type-header">
          ...
          <el-button
            ...
            icon="el-icon-plus"
            circle
            @click="addNewTable(items.id)"
          />
          <el-button plain type="primary" size="mini" icon="el-icon-edit" circle @click="addNewType(items)" />
          <el-button
            plain
            type="danger"
            size="mini"
            icon="el-icon-delete"
            circle
            @click="deleteTableType(items)"
          />
        </div>
        <el-row :gutter="20">
          <el-col v-for="(item, i) in items.children" :key="i" :span="8">
    ...
    <AddNewTableClassify :is-show.sync="isShowAddTable" :pid="tablePid" />
    <AddNewTableType :is-show.sync="isShowAddType" :edit-data="editTypeData" />
    <EditCard :is-show.sync="isShowEditCard" :pid="pidForCardData" />
  </div>
</template>
<script>
import { Classify as masterApi } from '@/api/data-map'
import AddNewTableClassify from '../datacenter/AddNewTableClassify'
import AddNewTableType from '../datacenter/AddNewTableType'
import EditCard from '../datacenter/EditCard'
  ...
  methods: {
    ...
    deleteCard(data) {
      this.$confirm('是否确认删除当前卡片?', '提示', {
        ...
        masterApi.delete(data).then(res => {
          if (res.code === 0) {
            this.$message.success('删除成功')
            this.fetchTableClassifyTree()
          } else {
    ...
    checkDetails(data) {
      this.$router.push({ name: 'CMDBClassifyInfo', query: { pid: data }})
    }
  }
}
</script>
