          ...
          >
            ...
            <el-table-column align="center" class-name="small-padding fixed-width" label="操作" width="360">
              <template slot-scope="{row}">
                <el-button size="mini" type="primary" @click="acceptFormVisible(row)">
                  接受
                </el-button>
...
export default {
  ...
  methods: {
    // 接受函数
    acceptFormVisible(val) {
      const parems = { minion: val.name }
      masterApi.create(parems).then((res) => {
        this.getList()
        this.$notify({
          title: '成功',
          message: '已接受key',
    ...
    // 获取 key 列表
    getList() {
      masterApi.listunaccept(this.pageQuerylist).then(res => {
