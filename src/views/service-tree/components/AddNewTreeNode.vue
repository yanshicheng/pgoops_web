<template>
  <el-dialog :title="title" :visible="isShow" width="450px" :close-on-click-modal="false" @close="resetForm('ruleForm')">
    <el-form ref="ruleForm" :model="ruleForm" :size="size" :rules="rules" label-width="100px" class="demo-ruleForm">
      <el-form-item label="节点名称" prop="name" required>
        <el-input v-model="ruleForm.name" placeholder="请输入节点名称" />
      </el-form-item>
      <el-form-item label="中文名称" prop="name_cn">
        <el-input v-model="ruleForm.name_cn" placeholder="请输入中文名称" />
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    editNodeTree() {
      ...
      masterApi.update(this.editData.pk, params).then(data => {
        if (data.code !== 0) {
    ...
    addNodeTree() {
      ...
      masterApi.create(params).then(data => {
        if (data.code !== 0) {
          this.$message.error(data.message)
          return false
        }
