<template>
  <div class="nav-tree">
    <div class="operate-group">
      ...
      <el-input v-model="keyword" placeholder="输入关键字进行过滤" class="input-with-select" size="small" @keyup.enter.native="getServiceTreeList">
        <el-select slot="prepend" v-model="select" placeholder="请选择" style="width:100px">
          <el-option label="节点名称" value="node" />
    ...
    >
      <span slot-scope="{ node, data }" class="custom-tree-node" @mouseover="nodeId = node.id">
        ...
        <span class="operate" @click="nodeId = node.id">
          <el-popover placement="bottom" trigger="click" @show="nodeId = node.id" @hide="nodeId = null">
            ...
            <i v-show="nodeId === node.id" slot="reference" class="el-icon-circle-plus-outline" />
          </el-popover>
        </span>
      </span>
    </el-tree>
    <AddNewTreeNode :is-show.sync="isShow" :edit-data="editData" />
    <TagManageDia :is-show.sync="isShowTagManage" :node-id="nodeId" />
  </div>
</template>

<script>
import { ServiceTree as masterApi } from '@/api/server-tree'
  ...
  methods: {
    ...
    deleteNode(node, data) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        masterApi.delete(data.pk).then(response => {
          if (response.code === 0) {
            this.getServiceTreeList()
