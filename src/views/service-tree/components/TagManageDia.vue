<template>
  <el-dialog :title="title" :visible="isShow" width="450px" :close-on-click-modal="false" @close="closeDia">
...
<script>
import { ServiceTreeTag as masterApi } from '@/api/server-tree'
...
export default {
  ...
  methods: {
    ...
    getTagList(id) {
      ServiceTree.gettag(id).then(res => {
        this.dynamicTags = res.data.tag
      })
    },
    deleteNodesTag(tag) {
      masterApi.delete(tag.id).then(res => {
